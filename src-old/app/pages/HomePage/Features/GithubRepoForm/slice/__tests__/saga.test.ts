...
describe('getRepos Saga', () => {
    ...
    it('should return error if username is empty', () => {
        ...
        expect(putDescriptor).toEqual(
            put(
                slice.githubRepoFormActions.repoError(
                    RepoErrorType.USERNAME_EMPTY,
                ),
            ),
        );
    ...
    it('should dispatch the user has no repo error', () => {
        ...
        expect(putDescriptor).toEqual(
            put(
                slice.githubRepoFormActions.repoError(
                    RepoErrorType.USER_HAS_NO_REPO,
                ),
            ),
        );
    });
    it('should dispatch the github rate limit error', () => {
        ...
        expect(putDescriptor).toEqual(
            put(
                slice.githubRepoFormActions.repoError(
                    RepoErrorType.GITHUB_RATE_LIMIT,
                ),
            ),
        );
    });
    ...
    it('should dispatch the response error', () => {
        ...
        const putDescriptor = getReposIterator.throw(new Error('some error'))
            .value;
        expect(putDescriptor).toEqual(
            put(
                slice.githubRepoFormActions.repoError(
                    RepoErrorType.RESPONSE_ERROR,
                ),
            ),
        );
    });
